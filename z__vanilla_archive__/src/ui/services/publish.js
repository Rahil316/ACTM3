...
function renderLoadingOverlay() {
  ...
  slot.innerHTML = '';
  slot.appendChild(
    el('div', { class: 'w-12 h-12 border-4 border-[var(--accent)] border-t-transparent rounded-full animate-spin' }),
  );
  slot.appendChild(el('p', { class: 'text-xl font-bold text-[var(--text-primary)]' }, 'Creating Variables...'));
  slot.appendChild(
    el('p', { class: 'text-[var(--text-muted)]' }, 'Generating color tokens and thematic variations in Figma.'),
  );
}
...
function renderSuccessDialog(tally) {
  ...
  const iconEl = el('div', {
    class: 'w-20 h-20 bg-green-500/10 rounded-full flex items-center justify-center text-green-500',
  });
  ...
  if (tally) {
    [
      ...
      resultsEl.appendChild(
        el('p', { class: 'text-sm' }, [`${label}: `, el('span', { class: `${cls} font-bold` }, String(count))]),
      );
    });
  }
...
function renderValidationWarningDialog(issues, onContinue) {
  ...
  slot.appendChild(
    el(
      'div',
      { class: 'w-16 h-16 bg-[var(--warning)]/10 rounded-full flex items-center justify-center text-[var(--warning)]' },
      [Icons.AlertTriangle],
    ),
  );
  slot.appendChild(
  ...
  slot.appendChild(
    el(
      'p',
      { class: 'text-[var(--text-muted)] text-[12px] text-center' },
  ...
  issues.forEach((msg) => {
    list.appendChild(
      el(
        ...
        {
          class:
            'text-[12px] text-[var(--text-secondary)] bg-[var(--warning)]/5 border border-[var(--warning)]/20 rounded-[6px] px-3 py-2',
        },
        msg,
      ),
    );
...
function renderErrorDialog(message) {
  ...
  const iconEl = el('div', {
    class: 'w-20 h-20 bg-red-500/10 rounded-full flex items-center justify-center text-red-500',
  });
...
function renderRunDialog() {
  ...
  const header = el('div', { class: 'px-4 py-3 flex items-center justify-between border-b border-[var(--border)]' }, [
    el('h2', { class: 'text-[17px] font-bold text-[var(--text-primary)]' }, 'Publish to Figma'),
    el(
      'button',
      {
  ...
  const body = el('div', { class: 'flex-1 overflow-y-auto p-4 space-y-5' }, [
    ...
    el('div', { class: 'space-y-2' }, [
      ...
      el(
        'div',
        {
          id: 'embed-colors-directly',
          class:
            'flex items-center justify-between p-3 bg-[var(--bg-card)] rounded-[8px] border border-[var(--border)]',
        },
        [
          el('div', {}, [
            ...
            el(
              ...
              'Write hex values into tokens instead of referencing the color scales',
            ),
          ]),
    ...
    el(
      ...
      {
        id: 'rd-warnings',
        class: 'hidden bg-[var(--warning)]/10 border border-[var(--warning)]/30 rounded-[8px] p-3 space-y-1',
      },
      [
...
function _buildImportWarningIcon() {
  return el(
    'div',
    { class: 'w-20 h-20 bg-[var(--warning)]/10 rounded-full flex items-center justify-center text-[var(--warning)]' },
    [Icons.AlertTriangle],
  );
}
...
function proceedWithSync() {
  ...
  setTimeout(() => {
    parent.postMessage(
      { pluginMessage: { type: 'run-creator', state: appState, scope: pendingScope, savedState: getSavedState() } },
      '*',
    );
  }, 50);
}
...
function refreshRunDialog() {
  ...
  if (colsEl) {
    ...
    if (entries.length) {
      entries.forEach(([name, label, isExisting]) => {
        colsEl.appendChild(
          el(
            ...
            {
              class:
                'flex items-center justify-between bg-[var(--bg-card)] border border-[var(--border)] rounded-[8px] px-3 py-2',
            },
            [
      ...
      colsEl.appendChild(
        el(
          'p',
          { class: 'text-[12px] text-[var(--text-muted)] px-1' },
          'No collections will be modified for this scope.',
        ),
      );
    }
  }
  ...
  const stepLabel = sampleVar
    ? appState.useShorthandVariations && sampleVar.shorthand
      ? sampleVar.shorthand
      : sampleVar.name
    : 'default';
  ...
  if (renameEl && renameListEl) {
    ...
    if (scaleCount + tokenCount > 0 && changes.length > 0) {
      ...
      const typeLabels = {
        color: 'Color',
        role: 'Role',
        stepNames: 'Scale Steps',
        roleStepNames: 'Variation Levels',
        grouping: 'Grouping',
      };
      changes.forEach((ch) => {
        renameListEl.appendChild(
          el(
            ...
            {
              class:
                'flex items-center gap-2 bg-[var(--bg-card)] border border-[var(--border)] rounded-[8px] px-3 py-2 min-w-0',
            },
            [
              el(
                'span',
                { class: 'text-[11px] text-[var(--text-muted)] w-[68px] shrink-0' },
    ...
    rows.forEach(([label, value]) => {
      sumEl.appendChild(
        el(
          ...
          {
            class:
              'flex items-start justify-between gap-2 text-[12px] py-1 border-b border-[var(--border)]/40 last:border-0',
          },
          [
  ...
  if (warnEl) {
    const relevant = existing.filter(
      (n) => (n === colorName && !skipScales && scope !== 'roles') || (n === tokenColName && scope !== 'groups'),
    );
...
const FORMAT_DEFS = [
  ...
  },
  {
    ...
    files: (themes) => [
      ...
      ...themes.slice(1).map((t) => 'android/res/values-' + t + '/colors.xml'),
    ],
  },
  {
    id: 'rn-ts',
...
function renderFormatCard(def) {
  ...
  const fileTree = el(
    ...
    {
      class:
        'mt-1.5 bg-[var(--bg-app)] rounded-[6px] px-2.5 py-2 font-mono text-[10px] text-[var(--text-muted)] leading-[1.7]',
    },
...
function renderExportSheet() {
  ...
  if (zipBtn) {
    ...
    zipBtn.textContent =
      count > 0 ? 'Export ' + count + ' Format' + (count > 1 ? 's' : '') + ' as ZIP' : 'Select formats above';
