...
There is no test suite (`test`/`test:watch` are not wired up despite being mentioned in `_Documentations/COMMANDS.md` — that doc is stale on this point).
...
- Project-specific design/domain knowledge (color role naming, contrast target conventions, algorithm selection guidance, feature status, outstanding todos) lives in `_Documentations/knowledge/` — check `_Documentations/knowledge/MEMORY.md` for the index before starting nontrivial feature or design work.
