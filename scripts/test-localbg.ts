...
import { variableMaker, type EngineConfig } from '../src/shared/clrEngine.js';
...
const config: EngineConfig = {
  ...
  roles: [
    ...
    },
    {
      ...
      // Token name pattern = "brand-1-fill-0" / "brand-2-fill-0"
      localBg: null,
...
const pass1 = variableMaker(config as EngineConfig);
const resolved = resolveTokenRefBgs(config, pass1);
const result   = resolved ? variableMaker(config as EngineConfig) : pass1;
...
for (const r of config.roles) {
  const targets = (r.variationTargets ?? []).join(', ');
  const vars    = (config.variations ?? []).map(v => v.name).join(', ');
  const lbg     = r.localBgDynamicRef
    ? `dynamic: "${r.localBgDynamicRef}"`
    : r.localBg
    ? JSON.stringify(r.localBg)
    : 'none (theme bg)';
...
if (result.errors.warnings.length > 0) {
  ...
  for (const w of result.errors.warnings)
    console.log(`     ${w.theme} / ${w.color} / ${w.role}: ${w.warning}`);
}
...
console.log(
  '  ' +
  row(['Theme',W.theme],['Color',W.color],['Role',W.role],
      ['BG used',W.bg],['Target',W.target],['Step',W.step],
      ['Hex',W.hex],['Contrast',W.ratio],['Adj',W.adj])
);
...
for (const [themeName, colorMap] of Object.entries(result.tokens)) {
  ...
  if (lastTheme && lastTheme !== themeName)
    console.log('  ' + hr([W.theme,W.color,W.role,W.bg,W.target,W.step,W.hex,W.ratio,W.adj]));
  lastTheme = themeName;
  ...
  for (const [colorName, roleMap] of Object.entries(colorMap as Record<string, any>)) {
    for (const [roleIdxStr, varMap] of Object.entries(roleMap as Record<string, any>)) {
      ...
      for (const [, token] of Object.entries(varMap as Record<string, any>)) {
        const target   = role.variationTargets?.[parseInt(token.variation)] ?? 4.5;
        const ratio    = token.contrast?.ratio?.toFixed(2) ?? '—';
        const adjusted = token.isAdjusted ? '↺' : '✓';
        console.log(
          '  ' +
          row([themeName,         W.theme],
              [colorName,         W.color],
              [role.name,         W.role],
