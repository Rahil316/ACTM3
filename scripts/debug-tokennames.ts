...
const config: any = {
  name: 'debug', pluginMode: 'scale', scaleAlgorithm: 'Natural', scaleLength: 11,
  useUniformAlgorithm: true, algorithmScopeLevel: 'color', solverMode: 'natural',
  tokenNameSegments: ['color','role','variation'],
  useShorthandColors: false, useShorthandRoles: false,
  useShorthandVariations: false, useShorthandSteps: false,
  includeSourceColors: false, alphaValues: [], tokenGrouping: 'color',
  includeColorScalesCollection: true, includeDescriptions: false,
  scaleCollectionName: '_scale', tokenCollectionName: 'color tokens',
  scaleStepNames: undefined, perRoleVariationOverride: false,
  colors: [
    { _id: 'c-brand1', name: 'brand-1', shorthand: 'b1', value: '#5B6AF0' },
    { _id: 'c-brand2', name: 'brand-2', shorthand: 'b2', value: '#5B6AF0' },
  ],
  themes: [{ name: 'Light', bg: '#FFFFFF' }, { name: 'Dark', bg: '#0F172A' }],
  variations: [{ name: 'base', shorthand: 'b' }],
  roles: [
    { name: 'fill', shorthand: 'fi', mappingMethod: 'contrast', variationTargets: [5] },
    { name: 'TextOnFill', shorthand: 'tof', mappingMethod: 'contrast', variationTargets: [5],
      localBg: null, localBgDynamicRef: '[color]-fill-0' },
  ],
};
...
console.log('  Replacing [color] with color names:', config.colors.map((c:any) => c.name));
console.log('  Refs to look up:', config.colors.map((c:any) => `"${c.name}-fill-0"`));
