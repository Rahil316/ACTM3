...
function _pvTokenTile(token, varLabel, ink) {
  ...
  const footer = el("div", {
    ...
    onmouseenter: token.tokenName
      ? (e) => { e.currentTarget.style.background = `rgba(${ink},0.09)`; }
      : undefined,
    onmouseleave: token.tokenName
      ? (e) => { e.currentTarget.style.background = `rgba(${ink},0.04)`; }
      : undefined,
  }, footerChildren);
...
function _pvTableSection(sectionName, subsections, ink) {
  ...
  for (const [subKey, variations] of Object.entries(subsections)) {
    ...
    const subName     = isRoleKey
      ? ((appState.roles[subKey] && appState.roles[subKey].name) || `Role ${subKey}`)
      : subKey;
    ...
    rows.push(el("div", {
      style: [
        ...
        "height:26px",
        !firstSubsection ? `border-top:1px solid rgba(${ink},0.08)` : "",
      ].filter(Boolean).join(";"),
    }, [
      el("div", {
        style: [
    ...
    for (const [varKey, token] of Object.entries(variations)) {
      ...
      const row = el("div", {
        ...
        title: `${token.value.toUpperCase()} — click to copy hex`,
        onclick: () => { copyToClipboard(token.value); ToastManager.success(`Copied ${token.value.toUpperCase()}`); },
        onmouseenter: (e) => { e.currentTarget.style.background = `rgba(${ink},0.04)`; },
        ...
        dataCell(
          el("span", {
            ...
            onclick: (e) => { e.stopPropagation(); copyToClipboard(token.value); ToastManager.success(`Copied ${token.value.toUpperCase()}`); },
          }, token.value.toUpperCase()),
        ),
        // Ratio
        dataCell(
        ...
        dataCell(
          token.tokenName
            ? el("span", {
                ...
                onclick: (e) => { e.stopPropagation(); copyToClipboard(token.tokenName); ToastManager.success(`Copied ${token.tokenName}`); },
              }, token.tokenName)
...
function renderPreviewPanel(result) {
  ...
  const grpColor = _pvSegBtn("Color", _pvState.groupBy === "color", () => {
    ...
    grpColor.style.background = "var(--accent)"; grpColor.style.color = "#fff";
    grpRole.style.background  = "var(--bg-input)"; grpRole.style.color = "var(--text-muted)";
    repaintPanels();
  });
  const grpRole = _pvSegBtn("Role", _pvState.groupBy === "role", () => {
    _pvState.groupBy = "role";
    grpRole.style.background  = "var(--accent)"; grpRole.style.color = "#fff";
    grpColor.style.background = "var(--bg-input)"; grpColor.style.color = "var(--text-muted)";
    repaintPanels();
  });
  ...
  const vmGrid = _pvSegBtn("Grid", _pvState.viewMode === "grid", () => {
    ...
    vmGrid.style.background  = "var(--accent)"; vmGrid.style.color = "#fff";
    vmTable.style.background = "var(--bg-input)"; vmTable.style.color = "var(--text-muted)";
    repaintPanels();
  });
  const vmTable = _pvSegBtn("Table", _pvState.viewMode === "table", () => {
    _pvState.viewMode = "table";
    vmTable.style.background = "var(--accent)"; vmTable.style.color = "#fff";
    vmGrid.style.background  = "var(--bg-input)"; vmGrid.style.color = "var(--text-muted)";
    repaintPanels();
  });
