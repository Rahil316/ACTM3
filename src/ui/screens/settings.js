...
// ============================================================================
// ── 1. CARD COMPONENTS (SUB-RENDERERS) ──
// ============================================================================

function _buildModeCard() {
  return panelUI.card([
    ...
    el("div", { id: "setting-global-solver-row", class: "hidden space-y-1" }, [panelUI.selectInput("setting-solverMode", SOLVER_MODE_OPTIONS, "Solver")]),
...
function _buildPaletteCard() {
  return el("div", { id: "settings-scale-section", class: "settings-card space-y-3" }, [
    panelUI.sectionLabel("Palette"),
    el("div", { class: "grid grid-cols-2 gap-3" }, [panelUI.input({ id: "setting-scaleLength", type: "number", size: "xl", label: "Steps" })]),
  ]);
}

function _buildVariationsCard() {
  return panelUI.card([
      ...
      el("div", { class: "flex items-center justify-between" }, [
        ...
        el(
          "button",
          {
            ...
            class:
              "h-[28px] px-2 text-[11px] font-medium rounded-[6px] bg-transparent border border-transparent text-[var(--accent)] hover:bg-[var(--accent)]/10 cursor-pointer transition-all",
          },
          ["+ Add"],
        ),
      ]),
...
function _buildNamingCard() {
  return panelUI.card([
    panelUI.sectionLabel("Token Naming"),
    panelUI.smallRow(
      "Shorthand for Colors",
      panelUI.togglePill("toggle-useShorthandColors", () => toggleBoolSetting("useShorthandColors")),
    ),
    panelUI.smallRow(
      "Shorthand for Roles",
      panelUI.togglePill("toggle-useShorthandRoles", () => toggleBoolSetting("useShorthandRoles")),
    ),
    panelUI.smallRow(
      "Shorthand for Variations",
      panelUI.togglePill("toggle-useShorthandVariations", () => toggleBoolSetting("useShorthandVariations")),
    ),
    panelUI.smallRow(
      "Shorthand for Scale Steps",
      panelUI.togglePill("toggle-useShorthandSteps", () => toggleBoolSetting("useShorthandSteps")),
    ),
...
function _buildCollectionsCard() {
  return panelUI.card([
    ...
    el("div", { id: "settings-palettes-collection-group", class: "space-y-2" }, [
      ...
      el("div", { id: "settings-scale-collection-row" }, [panelUI.input({ id: "setting-scaleCollectionName", placeholder: "_Palettes", size: "lg" })]),
    ]),
...
function _buildStepLabelsCard() {
  return el("div", { id: "settings-step-labels-section", class: "settings-card space-y-3" }, [
    panelUI.sectionLabel("Scale Step Labels"),
    el("p", { class: "text-[11px] text-[var(--text-muted)] -mt-1" }, [
      "Name each step in the scale. Shorthand is used in token names when 'Shorthand for Scale Steps' is on.",
    ]),
    el("div", { class: "flex items-center justify-between" }, [
      ...
      el(
        "button",
        {
          ...
          class:
            "h-[28px] px-2 text-[11px] font-medium rounded-[6px] bg-transparent border border-transparent text-[var(--accent)] hover:bg-[var(--accent)]/10 cursor-pointer transition-all shrink-0",
        },
        ["+ Add"],
      ),
    ]),
...
function renderSettingsTokensPanel() {
  ...
  mount.appendChild(_buildModeCard());
  mount.appendChild(_buildPaletteCard());
  mount.appendChild(_buildVariationsCard());
  mount.appendChild(_buildNamingCard());
  mount.appendChild(_buildCollectionsCard());
  mount.appendChild(_buildStepLabelsCard());
}
...
function renderSettingsPluginPanel() {
  ...
  const scaleSelect = panelUI.selectInput(
    "setting-ui-scale",
    [
      ["1.0", "100% (default)"],
      ["0.7", "70%"],
  ...
  const themeSelect = panelUI.selectInput(
    "setting-ui-theme",
    [
      ["figma", "Follow Figma"],
      ["dark", "Dark"],
  ...
  const langSelect = panelUI.selectInput(
    "setting-ui-lang",
    [
      ["en", "English"],
      ["es", "Español"],
  ...
  const uiCard = panelUI.card([panelUI.sectionLabel(t("interface")), el("div", { class: "flex flex-col gap-3" }, [scaleSelect, themeSelect, langSelect])]);
  ...
  themes.forEach((theme, idx) => {
    const row = el("div", { class: `flex items-center gap-1.5 ${rowClass}` }, [
      ...
      panelUI.input({
        ...
        class: "w-24",
        oninput: (e) => {
          updateTheme(idx, "name", e.target.value);
          renderPreviewTabs();
        },
      }),
      inputsUI.colorInput(
...
// ============================================================================
// ── 4. STATE ↔ DOM SYNCHRONIZATION ──
// ============================================================================

function _syncTogglePills() {
  [
    "resolveTokensDirectly",
    "useShorthandColors",
    "useShorthandRoles",
    "useShorthandVariations",
    "useShorthandSteps",
    "includeSourceColors",
    "includeAlphaTints",
    "perRoleVariationOverride",
    "includeDescriptions",
    "useUniformAlgorithm",
    "includeColorScalesCollection",
  ].forEach((key) => {
...
function _syncNameFormatPreview() {
  ...
  previewEl.innerHTML = order.map((s) => `<span style="color:${_pillColor(s)};font-weight:600">${segValues[s] || s}</span>`).join(sep);
}
...
function openSettings() {
  _settingsSnapshot = JSON.parse(
    JSON.stringify({
      ...
      themes: appState.themes ? JSON.parse(JSON.stringify(appState.themes)) : null,
    }),
  );
  syncInputsFromState();
...
function renderTokenOrderPills() {
  ...
  order.forEach((segment, idx) => {
    ...
    pill.addEventListener("dragover", (e) => {
      e.preventDefault();
      if (_pillDragSrc !== null && _pillDragSrc !== idx) pill.style.boxShadow = `0 0 0 2px #fff8`;
    });
    pill.addEventListener("dragleave", () => {
      pill.style.boxShadow = `0 2px 8px ${c}55`;
    });
