...
import type { EngineResult, EngineConfig, TokenEntry, ScaleStep } from "../../shared/clrEngine";
...
const STATUS_ICON: Record<NodeStatus, string> = {
  ok: "", warn: "⚠", error: "✕", adjusted: "↺", notice: "ℹ",
};
const STATUS_ICON_COLOR: Record<NodeStatus, string> = {
  ok: "transparent", warn: "#fbbf24", error: "#f87171", adjusted: "#a78bfa", notice: "#60a5fa",
};
...
function TreeRow({
  ...
  return (
    <div>
      ...
      >
        <span style={{ width: 10, fontSize: 8, color: "#52525b", flexShrink: 0, userSelect: "none", lineHeight: 1 }}>
          {hasChildren ? (open ? "▼" : "▶") : ""}
        </span>

        {status !== "ok" && (
          <span style={{ fontSize: 9, color: STATUS_ICON_COLOR[status], flexShrink: 0, lineHeight: 1 }}>
            {STATUS_ICON[status]}
          </span>
        )}
        ...
        {hex && (
          <div style={{
            ...
            background: hex, border: `1px solid ${swatchBorder(hex)}`, flexShrink: 0,
          }} />
        )}
        ...
        {tag && (
          <span style={{
            ...
            borderRadius: 3, padding: "0 4px", color: "#71717a", flexShrink: 0,
          }}>
            {tag}
          </span>
        )}
...
function Field({ label, value, mono, color }: { label: string; value: string; mono?: boolean; color?: string }) {
  return (
    <div style={{ display: "flex", gap: 8, alignItems: "baseline", marginBottom: 2 }}>
      ...
      <span style={{ fontSize: 10, color: color ?? "#a1a1aa", fontFamily: mono ? "monospace" : undefined, wordBreak: "break-all" }}>
        {value}
      </span>
    </div>
  );
...
function FieldBlock({ depth, children }: { depth: number; children: React.ReactNode }) {
  return (
    <div style={{
      paddingLeft: depth * INDENT + 18, paddingTop: 4, paddingBottom: 6, paddingRight: 8,
      background: "rgba(255,255,255,0.025)", borderRadius: 4, marginBottom: 2,
    }}>
      {children}
    </div>
  );
  ...
  return (
    <div style={{ display: "flex", gap: 8, alignItems: "baseline", marginBottom: 2 }}>
      ...
      <span style={{ fontSize: 10, fontFamily: "monospace", color: contrastColor(ratio) }}>
        {ratio != null ? `${ratio.toFixed(2)}:1` : "—"}{rating ? ` ${rating}` : ""}
      </span>
    </div>
  );
}
  ...
  if (!hasAnything) {
    return (
      <div style={{
        ...
        marginBottom: 14, fontSize: 10, color: "#34d399",
      }}>
      ...
      {errors.critical.length > 0 && (
        <div style={{ padding: "8px 10px", borderRadius: 6, background: "rgba(248,113,113,0.1)", border: "1px solid rgba(248,113,113,0.35)", marginBottom: 6 }}>
          ...
          {errors.critical.map((c, i) => (
            <div key={i} style={{ fontSize: 10, color: "#fca5a5", marginBottom: 2 }}>{String(c)}</div>
          ))}
        </div>
      )}
        ...
        <div style={{ padding: "8px 10px", borderRadius: 6, background: "rgba(251,191,36,0.08)", border: "1px solid rgba(251,191,36,0.3)", marginBottom: 6 }}>
          ...
          {errors.warnings.map((w, i) => (
            <div key={i} style={{ fontSize: 10, color: "#fde68a", marginBottom: 2 }}>
              <span style={{ color: "#92400e" }}>{w.theme} / {w.color} / {w.role} / {w.variation} — </span>
              {w.warning}
            </div>
          ))}
        </div>
        ...
        <div style={{ padding: "8px 10px", borderRadius: 6, background: "rgba(96,165,250,0.08)", border: "1px solid rgba(96,165,250,0.25)" }}>
          ...
          {errors.notices.map((n, i) => (
            <div key={i} style={{ fontSize: 10, color: "#bfdbfe", marginBottom: 2 }}>
              <span style={{ color: "#1e3a5f" }}>{n.theme} / {n.color} / {n.role} / {n.variation} — </span>
              {n.notice}
            </div>
          ))}
        </div>
...
function ScaleStepNode({ stepKey, step, depth }: { stepKey: string; step: ScaleStep; depth: number }) {
  const hex = step.value;
  return (
    <TreeRow depth={depth} label={stepKey} hex={hex} defaultOpen={false}
      meta={<span style={{ fontSize: 9, fontFamily: "monospace", color: "#71717a" }}>{normalizeHex(hex)}</span>}
    >
...
function TokenEntryNode({
  varName, token, depth, warnings, notices,
}: {
  varName: string; token: TokenEntry; depth: number;
  warnings: EngineResult["errors"]["warnings"];
  notices: EngineResult["errors"]["notices"];
}) {
  ...
  const status: NodeStatus =
    myWarnings.length > 0 ? "warn"
    : token.isAdjusted    ? "adjusted"
    : myNotices.length > 0 ? "notice"
    : "ok";

  return (
    <TreeRow depth={depth} label={varName} hex={token.value} status={status}
      defaultOpen={false}
      tag={token.isAdjusted ? "adjusted" : undefined}
      meta={<span style={{ fontSize: 9, fontFamily: "monospace", color: contrastColor(ratio) }}>
    ...
    >
      <FieldBlock depth={depth}>
        ...
        {token.contrastTarget != null && (
          <Field label="contrastTarget" value={`${token.contrastTarget}:1`} mono
            color={ratio != null && ratio >= token.contrastTarget ? "#34d399" : "#f87171"} />
        )}
        <ContrastRow themeKey="bg" ratio={ratio} rating={token.contrast.rating} />
        {token.isAdjusted && (
          <Field label="isAdjusted" value="true — engine fell back to closest available step" color="#a78bfa" />
        )}
        {myWarnings.map((w, i) => <Field key={i} label={`warning ${i + 1}`} value={w.warning} color="#fbbf24" />)}
        {myNotices.map((n, i)  => <Field key={i} label={`notice ${i + 1}`}  value={n.notice}  color="#60a5fa" />)}
      </FieldBlock>
    </TreeRow>
  );
}
...
function RoleNode({
  roleName, roleIdx, varMap, depth, appState, result, colorName, themeName,
}: {
  roleName: string; roleIdx: number; varMap: Record<number, TokenEntry>;
  depth: number; appState: AppState; result: EngineResult; colorName: string; themeName: string;
}) {
  const entries = Object.entries(varMap);
  const roleObj = appState.roles[roleIdx];
  const varDefs = roleObj?.customVariationList && roleObj.customVariations?.length
    ? roleObj.customVariations : appState.variations ?? [];
  ...
  return (
    <TreeRow depth={depth} label={roleName} hex={firstHex} status={status}
      defaultOpen={false} tag={`${entries.length} var${entries.length !== 1 ? "s" : ""}`}
    >
      {entries.map(([varIdxStr, token]) => {
        const varDef = varDefs[parseInt(varIdxStr, 10)];
        return (
          <TokenEntryNode key={varIdxStr} varName={varDef?.name ?? `var-${varIdxStr}`}
...
function ThemeColorNode({
  colorName, roleMap, depth, appState, result, themeName, visibleRoles,
}: {
  colorName: string; roleMap: Record<number, Record<number, TokenEntry>>;
  depth: number; appState: AppState; result: EngineResult; themeName: string;
  ...
  const roleEntries = Object.entries(roleMap).filter(([roleIdxStr]) => {
    ...
    const roleObj = appState.roles[parseInt(roleIdxStr, 10)];
  ...
  return (
    <TreeRow depth={depth} label={colorName} hex={colorObj?.value} status={status}
      defaultOpen={false} tag={`${tokenCount} token${tokenCount !== 1 ? "s" : ""}`}
    >
      {roleEntries.map(([roleIdxStr, varMap]) => {
        const roleIdx = parseInt(roleIdxStr, 10);
        const roleObj = appState.roles[roleIdx];
        return (
          <RoleNode key={roleIdxStr} roleName={roleObj?.name ?? `role-${roleIdx}`}
            roleIdx={roleIdx} varMap={varMap} depth={depth + 1}
            appState={appState} result={result} colorName={colorName} themeName={themeName}
          />
        );
      })}
    </TreeRow>
...
function ConfigTree({ config, expandAll }: { config: EngineConfig; expandAll: boolean }) {
  return (
    <TreeRow depth={0} label="Config" defaultOpen={expandAll}
      tag={`${config.colors.length}c · ${config.roles.length}r · ${config.themes.length}t`}
    >
      {/* Global settings */}
      <TreeRow depth={1} label="Global" defaultOpen={expandAll}>
        <FieldBlock depth={1}>
          ...
          {config.scaleStepNames && config.scaleStepNames.length > 0 && (
            <Field label="scaleStepNames" value={config.scaleStepNames.join(", ")} mono />
          )}
        </FieldBlock>
      </TreeRow>

      {/* Colors */}
      <TreeRow depth={1} label="Colors" defaultOpen={expandAll}
        tag={`${config.colors.length}`}
      >
        {config.colors.map((c) => (
      ...
      {/* Themes */}
      <TreeRow depth={1} label="Themes" defaultOpen={expandAll}
        tag={`${config.themes.length}`}
      >
        {config.themes.map((t) => (
      ...
      {/* Roles */}
      <TreeRow depth={1} label="Roles" defaultOpen={expandAll}
        tag={`${config.roles.length}`}
      >
        {config.roles.map((r, i) => (
          <TreeRow key={r.name + i} depth={2} label={r.name} defaultOpen={false}>
            <FieldBlock depth={2}>
              ...
              {r.variationTargets && (
                <Field label="variationTargets" value={r.variationTargets.join(", ")} mono />
              )}
              ...
              {r.customVariationList && (
                <Field label="customVariations"
                  value={(r.customVariations ?? []).map((v) => v.name).join(", ")} />
              )}
              {r.scopedColorIds && r.scopedColorIds.length > 0 && (
                <Field label="scopedColorIds" value={r.scopedColorIds.join(", ")} mono />
              )}
              {r.localBg && (
                <Field label="localBg"
                  value={Object.entries(r.localBg).map(([k, v]) => `${k}:${v}`).join(", ")} mono />
              )}
            </FieldBlock>
          </TreeRow>
        ))}
      ...
      {/* Variations */}
      <TreeRow depth={1} label="Variations" defaultOpen={expandAll}
        tag={`${config.variations.length}`}
      >
        {config.variations.map((v, i) => (
          <TreeRow key={v.name + i} depth={2} label={v.name} defaultOpen={false}>
            <FieldBlock depth={2}>
              {v.shorthand && <Field label="shorthand" value={v.shorthand} mono />}
            </FieldBlock>
          </TreeRow>
        ))}
      </TreeRow>
...
export function CanvasPreviewDevTree({
  appState,
  config,
  result,
}: {
  ...
  const visibleColors: Set<string> | null = useMemo(() => {
    ...
    return names;
  }, [colorQ, activeChips, appState.colors, result.errors]);
  ...
  const visibleRoles: Set<string> | null = useMemo(() => {
    ...
    appState.roles.forEach((r) => {
      ...
      const matchAdjusted = !activeChips.has("adjusted") || (() => {
        return Object.values(result.tokens).some((colorMap) =>
          Object.values(colorMap).some((roleMap) => {
            ...
            return Object.entries(rm).some(([roleIdxStr]) => {
              ...
              const roleObj = appState.roles[idx];
    ...
    return names;
  }, [roleQ, activeChips, appState.roles, result.tokens, result.errors]);
  ...
  const totalAdjusted = useMemo(() =>
    Object.values(result.tokens).reduce((acc, colorMap) =>
      acc + Object.values(colorMap).reduce((a2, roleMap) =>
        a2 + Object.values(roleMap as object).reduce((a3: number, varMap: unknown) =>
          a3 + Object.values(varMap as object).filter((t: unknown) => (t as TokenEntry).isAdjusted).length, 0), 0), 0),
  [result.tokens]);
  ...
  // Filter scale colors
  const visibleScaleColors = visibleColors
    ? scaleEntries.filter(([name]) => visibleColors.has(name))
    : scaleEntries;
    ...
    <div style={{ fontFamily: "Inter, system-ui, sans-serif", fontSize: 11 }}>
      ...
      <div style={{
        ...
        <input
          ...
          style={{
            ...
            outline: "none", width: 100, fontFamily: "Inter, system-ui, sans-serif",
          }}
        />
        <input
          value={roleSearch}
        ...
        {(colorSearch || roleSearch) && (
          <ToolbarBtn label="✕ Clear" onClick={() => { setColorSearch(""); setRoleSearch(""); }} />
        )}
        ...
        <Chip
          ...
          active={activeChips.has("warnings")} color="#fbbf24"
          onClick={() => toggleChip("warnings")}
        />
        <Chip
          label={`✕ ${result.errors.critical.length} critical`}
          active={activeChips.has("errors")} color="#f87171"
          onClick={() => toggleChip("errors")}
        />
        <Chip
          label={`↺ ${totalAdjusted} adjusted`}
          active={activeChips.has("adjusted")} color="#a78bfa"
          onClick={() => toggleChip("adjusted")}
        />
        <Chip
          label={`ℹ ${totalNotices} notice${totalNotices !== 1 ? "s" : ""}`}
          active={activeChips.has("warnings")} color="#60a5fa"
          onClick={() => toggleChip("warnings")}
        />

        {activeChips.size > 0 && (
          <ToolbarBtn label="✕ Reset filters" onClick={() => setActiveChips(new Set())} />
        )}
      </div>
      ...
      <div key={expandKey}>
        ...
        {includeScales && visibleScaleColors.length > 0 && (
          <TreeRow depth={0} label="Scales" defaultOpen={expandAll}
            tag={`${visibleScaleColors.length} color${visibleScaleColors.length !== 1 ? "s" : ""}`}
          >
            {visibleScaleColors.map(([colorName, steps]) => {
              const colorObj = appState.colors.find((c) => c.name === colorName);
              const stepEntries = Object.entries(steps);
              return (
                <TreeRow key={colorName} depth={1} label={colorName} hex={colorObj?.value}
                  defaultOpen={expandAll}
                  tag={`${stepEntries.length} step${stepEntries.length !== 1 ? "s" : ""}`}
                >
        ...
        {/* Tokens */}
        <TreeRow depth={0} label="Tokens" defaultOpen={true}
          tag={`${themeEntries.length} theme${themeEntries.length !== 1 ? "s" : ""}`}
          status={totalWarnings > 0 ? "warn" : totalAdjusted > 0 ? "adjusted" : "ok"}
        >
          {themeEntries.map(([themeName, colorMap]) => {
            const theme = appState.themes.find((t) => t.name.toLowerCase() === themeName);

            // apply color filter to this theme's color entries
            const colorEntries = Object.entries(colorMap).filter(([cn]) =>
              !visibleColors || visibleColors.has(cn),
            );
            if (colorEntries.length === 0) return null;

            const themeWarn     = result.errors.warnings.some((w) => w.theme === themeName);
            const themeAdjusted = colorEntries.some(([, roleMap]) =>
              Object.values(roleMap as object).some((vm: unknown) =>
                Object.values(vm as object).some((t: unknown) => (t as TokenEntry).isAdjusted),
              ),
            );

            return (
              <TreeRow key={themeName} depth={1} label={themeName} hex={theme?.bg}
                defaultOpen={expandAll}
                status={themeWarn ? "warn" : themeAdjusted ? "adjusted" : "ok"}
                tag={`${colorEntries.length} color${colorEntries.length !== 1 ? "s" : ""}`}
              >
                {colorEntries.map(([colorName, roleMap]) => (
                  <ThemeColorNode
                    ...
                    depth={2} appState={appState} result={result}
                    themeName={themeName} visibleRoles={visibleRoles}
                  />
                ))}
              </TreeRow>
            );
