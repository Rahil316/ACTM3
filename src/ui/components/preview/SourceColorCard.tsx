...
export function SourceColorCard({ color, alphaValues = '', showAlphas = false }: SourceColorCardProps) {
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      {/* Swatch tile */}
      <div
        className="rounded-[10px] overflow-hidden border border-border-base cursor-pointer hover:opacity-90 transition-opacity"
      ...
      >
        <div className="h-16 flex items-end p-3" style={{ background: hex }}>
          <span
            ...
            style={{ color: inkColor(swatchInk, 0.85), textShadow: '0 1px 3px rgba(0,0,0,0.15)' }}
          >
            {hex.toUpperCase()}
          </span>
        </div>
        <div className="px-3 py-2 flex items-center justify-between bg-bg-input border-t border-border-subtle">
          ...
          {color.shorthand && (
            <span className="text-[10px] font-mono text-text-dim">{color.shorthand}</span>
          )}
        </div>
      </div>
            ...
            {alphas.map((alpha) => {
              ...
              return (
                <div
                  ...
                  onClick={() => copyText(rgba, `${color.name} ${alpha}%`)}
                >
                  <span
                    ...
                    style={{ color: inkColor(swatchInk) }}
                  >
                    {alpha}%
                  </span>
                </div>
