...
const BASE_INPUT =
  'w-full bg-bg-input text-text-primary border outline-none transition-colors ' +
...
function InputWrapper({
  id, label, hint, state, infoIcon, children, width,
}: {
  id: string;
  label?:    string;
  hint?:     string;
  ...
  return (
    <div className={clsx('flex flex-col gap-1', width)}>
      ...
      {children}
      {hint && (
        <p className={clsx('text-[11px] ml-0.5', STATE_HINT[state])}>{hint}</p>
      )}
    </div>
  );
}
...
function WithIcons({
  children, leading, trailing, size,
}: {
  children: ReactNode;
  leading?:  ReactNode;
  trailing?: ReactNode;
  ...
  return (
    <div className="relative flex items-center">
      {leading && (
        <span className={clsx('absolute left-2.5 flex items-center justify-center text-text-muted pointer-events-none shrink-0', iconSize)}>
          {leading}
        </span>
      )}
      {children}
      {trailing && (
        <span className={clsx('absolute right-2.5 flex items-center justify-center text-text-muted pointer-events-none shrink-0', iconSize)}>
          {trailing}
        </span>
      )}
    </div>
