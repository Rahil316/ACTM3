...
export function Button({ variant = "secondary", size = "lg", radius = "md", square, label, leftIcon, rightIcon, icon, loading = false, fullWidth = false, disabled, className, children, ...rest }: ButtonProps) {
  ...
  return (
    <button {...rest} disabled={isDisabled} className={clsx("inline-flex items-center justify-center font-medium transition-all border select-none outline-none", sizeCls, radCls, VARIANT[variant], isDisabled && "opacity-40 cursor-not-allowed pointer-events-none", fullWidth && "w-full", className)}>
      {loading ? (
        ...
        <>
          {(leftIcon || icon) && <span className="inline-flex items-center justify-center shrink-0">{leftIcon ?? icon}</span>}
...
function Spinner({ size }: { size: ButtonSize }) {
  ...
  return (
    <svg width={s} height={s} viewBox="0 0 16 16" fill="none" className="animate-spin" style={{ animationDuration: "0.7s" }}>
