...
export const Header: StoryObj<typeof MenuHeader> = {
  render: (args) => (
    <div className="max-w-sm border border-border-base rounded-lg bg-bg-card overflow-hidden">
      <MenuHeader {...args} />
    </div>
  ),
  args: {
    ...
    <div className="max-w-sm border border-border-base rounded-lg bg-bg-card overflow-hidden">
      <MenuRow {...args}>
        ...
        <div className="flex flex-col min-w-0">
          <span className="text-[12px] font-semibold text-text-primary">Brand/Primary</span>
          <span className="text-[10px] text-text-muted">Primary Brand Color · #0066FF</span>
        </div>
      </MenuRow>
    </div>
  ),
...
export const InteractiveSuggestSheet: StoryObj = {
  render: () => {
    ...
    return (
      <div className="p-4 max-w-sm border border-border-base rounded-lg bg-bg-app relative min-h-[300px] flex items-center justify-center">
        ...
        {open && (
          ...
          >
            <MenuRow onClick={() => { alert('Selected Crimson'); setOpen(false); }}>
              ...
              <span className="text-[12px] text-text-primary font-medium">Crimson</span>
            </MenuRow>
            <MenuRow onClick={() => { alert('Selected Emerald'); setOpen(false); }}>
              <ColorSwatch color="#059669" size="sm" />
              <span className="text-[12px] text-text-primary font-medium">Emerald</span>
            </MenuRow>
            <MenuRow onClick={() => { alert('Selected Indigo'); setOpen(false); }}>
              <ColorSwatch color="#4F46E5" size="sm" />
              <span className="text-[12px] text-text-primary font-medium">Indigo</span>
            </MenuRow>
          </SuggestSheet>
        )}
      </div>
