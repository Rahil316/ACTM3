...
If you want a *configuration file* instead of code (naming/sorting/
grouping rules declared as JSON5, no code to run), use the `"custom"`
format and [`fmt-lang/README.md`](../../../fmt-lang/README.md) instead —
that's a genuinely different tool for a genuinely different need. This
document is only about `"script"`.
...
"@rahil316/token-wand/dist/shared/exportEng/scriptExport"` gives you full
autocomplete/type-checking while writing it; only the file the CLI actually
loads at build time needs to be plain JS.
