 ...
 * │  variableMaker(config)                         → { colorRamps,           │
 * │                                                    colorTokens, errors }  │
 * └───────────────────────────────────────────────────────────────────────────┘
 * ============================================================================
 */
...
const TONAL_SCALE_ALGO = {
  ...
  Expressive: (hue, satu, N, stepLum, findL) => {
    const tapS   = (L) => satu * (1 - Math.pow(Math.abs(L - 50) / 50, 1.5) * 0.4);
    const shiftH = (L) => { const d = (L - 50) / 50; return (hue + shortestHueDiff(hue, d > 0 ? 60 : 240) * Math.abs(d) * 0.15 + 360) % 360; };
    const out = [];
    for (let i = 0; i < N; i++) {
 ...
 * @param {number}   config.scaleLength      - ramp step count
 ...
 * @param {string}   config.pluginMode       - "tonal" | "direct"
 ...
 * @returns {{ colorRamps: object, colorTokens: object, errors: object }}
 */
function variableMaker(config) {
  ...
  const clrRamps = config.pluginMode !== "direct"
    ? _generateBaseRamps(colors, scaleLength, config.scaleAlgorithm, config.scaleStepNames, lightBg, darkBg)
    : Object.create(null);
  ...
  return { colorRamps: clrRamps, colorTokens: tokensCollection, errors };
}

/**
 * Build tonal ramps for all colors. Each step stores value, naming, and
 * contrast ratios against both light and dark backgrounds.
 * @returns {object} { [colorName]: { [stepName]: { value, stepName, shorthand, description, contrast } } }
 */
function _generateBaseRamps(colors, scaleLength, scaleAlgo, stepNames, lightBg, darkBg) {
...
function _mapByContrastTarget(color, role, variations, ramp, stepNames, modeName, output, errors) {
  ...
  variations.forEach((_, vi) => {
    ...
    stepNames.forEach((name, si) => { const diff = Math.abs(ramp[name].contrast[modeName].ratio - targetC); if (diff < bestDiff) { bestDiff = diff; bestIdx = si; } });
    const data = ramp[stepNames[bestIdx]];
    output[vi] = { tknName: `${color.name}-${role.name}-${vi}`, color: color.name, role: role.name, variation: String(vi), roleDescription: role.description || "", tknRef: data.stepName, value: data.value, contrast: { ratio: data.contrast[modeName].ratio, rating: data.contrast[modeName].rating }, contrastTarget: targetC, isAdjusted: bestDiff > 0.5 };
...
function _mapByStepOffset(color, role, variations, ramp, stepNames, modeName, output, mappingType, errors) {
  ...
  variations.forEach((_, vi) => {
    ...
    const data     = ramp[stepNames[idx]];
    output[vi]     = { tknName: `${color.name}-${role.name}-${vi}`, color: color.name, role: role.name, variation: String(vi), roleDescription: role.description || "", tknRef: data.stepName, value: data.value, contrast: { ratio: data.contrast[modeName].ratio, rating: data.contrast[modeName].rating }, isAdjusted: adjusted };
  });
}
...
function _findBaseIndexByContrast(role, ramp, stepNames, modeName, isDark, colorName, roleName, errors) {
  ...
  let baseIdx = -1;
  if (isDark) {
    for (let i = stepNames.length - 1; i >= 0; i--) { if (ramp[stepNames[i]].contrast[modeName].ratio >= minC) { baseIdx = i; break; } }
  } else {
    for (let i = 0; i < stepNames.length; i++) { if (ramp[stepNames[i]].contrast[modeName].ratio >= minC) { baseIdx = i; break; } }
  }
  if (baseIdx === -1) {
    let bestIdx = 0, maxC = -1;
    stepNames.forEach((name, i) => { const c = ramp[name].contrast[modeName].ratio; if (c > maxC) { maxC = c; bestIdx = i; } });
    baseIdx = bestIdx;
...
const _M1i = [[4.0767416621, -3.3077115913, 0.2309699292], [-1.2684380046, 2.6097574011, -0.3413193965], [-0.0041960863, -0.7034186147, 1.707614701]];
// LMS^(1/3) ↔ Lab  (M2 forward, M2i inverse)
const _M2  = [[0.2104542553, 0.793617785, -0.0040720468], [1.9779984951, -2.428592205, 0.4505937099], [0.0259040371, 0.7827717662, -0.808675766]];
const _M2i = [[1.0, 0.3963377774, 0.2158037573], [1.0, -0.1055613458, -0.0638541728], [1.0, -0.0894841775, -1.291485548]];
...
function oklchToHex(L, C, H) {
  ...
  const [r, g, bl] = _m3(_M1i, _m3(_M2i, [L, a, b]).map((v) => v * v * v));
  return _lr2h(r, g, bl);
}
...
const _VC = (() => {
  ...
  const F  = 1, c = 0.69, Nc = 1;   // average surround
  const k  = 1 / (5 * aL + 1);
  ...
  const cat = [[0.7328, 0.4296, -0.1624], [-0.7036, 1.6975, 0.0061], [0.003, 0.0136, 0.9834]];
  const ci  = [[1.0961238208, -0.2788690002, 0.1827452039], [0.4543690419, 0.4735331543, 0.0720978039], [-0.0096276087, -0.0056980312, 1.0153256399]];
  const hpi = [[1.9101968341, -1.1121238928, 0.2019079568], [0.3709500882, 0.6290542574, -0.0000080551], [0, 0, 1]];
  ...
  const rW   = m3(cat, W.map((v) => v / 100));
  const Drgb = rW.map((v) => D / v + 1 - D);
  const ad   = (c2) => { const f = (FL * Math.abs(c2)) ** 0.42; return (400 * Math.sign(c2) * f) / (f + 27.13); };
  const aW   = m3(hpe, m3(ci, rW.map((v, i) => v * Drgb[i]))).map(ad);
...
function _hctRgbOrNull(hue, ch, J) {
  ...
  let a = 0, b = 0;
  if (ta > 0) { const g = (23 * (p2 + 0.305) * ta) / (23 * p1 + 11 * ta * Math.cos(hr) + 108 * ta * Math.sin(hr)); a = g * Math.cos(hr); b = g * Math.sin(hr); }
  const iv  = (c2) => { const s = Math.sign(c2); return (s * Math.pow(Math.max(0, (Math.abs(c2) * 27.13) / (400 - Math.abs(c2))), 1 / 0.42)) / v.FL; };
  const Ra  = (460 * p2 + 451 * a + 288 * b) / 1403;
  const Ga  = (460 * p2 - 891 * a - 261 * b) / 1403;
  const Ba  = (460 * p2 - 220 * a - 6300 * b) / 1403;
  const lr  = m3(_XL, m3(v.ci, m3(v.hpi, [Ra, Ga, Ba].map(iv)).map((c2, i) => c2 / v.Drgb[i])));
...
function hctToHex(hue, ch, tone) {
  ...
  let lo = 0, hi = ch, best = null;
  for (let it = 0; it < 50; it++) {
    if (hi - lo < 0.01) break;
    const mid = (lo + hi) / 2;
    const rgb = _hctRgbOrNull(hue, mid, J);
    if (rgb === null) { hi = mid; } else { best = _lr2h(rgb[0], rgb[1], rgb[2]); lo = mid; }
  }
...
function _relLumFromLinear(r, g, b) { return 0.2126 * r + 0.7152 * g + 0.0722 * b; }

/** WCAG contrast ratio from two relative luminances */
function _wcagContrast(lum1, lum2) { const hi = Math.max(lum1, lum2), lo = Math.min(lum1, lum2); return (hi + 0.05) / (lo + 0.05); }

/** Relative luminance of a hex string (skips double-linearization by reusing _h2lr) */
function _lumOfHex(hex) { const [r, g, b] = _h2lr(hex); return _relLumFromLinear(r, g, b); }
...
function _targetChroma(L, srcL, srcC, _srcH, mode) {
  ...
  switch (mode) {
    case "saturated":  return srcC; // hold source C, only move L
    case "luminance":  return srcC * (1 - Math.pow(Math.abs(2 * L - 1), 1.5)); // parabolic drop at extremes
    case "natural":    return (srcC / Math.max(srcL, 1 - srcL)) * Math.min(L, 1 - L); // linear through srcC at srcL
    default:           return srcC;
  }
}
...
function solveColorForContrast(sourceHex, targetContrast, bgHex, solverMode) {
  ...
  let solvedL = null, solvedC = null, chromaReduced = false;

  if (solverMode === "chroma-maximized") {
    // Solve L for contrast, then push C as high as gamut allows at that L
    const getHex = (L) => { const maxC = _maxChromaAtLH(L, src.H, Math.max(src.C, 0.2)); return oklchToHex(L, maxC < 0.001 ? 0 : maxC, src.H); };
    ...
    const getHex = (L) => { const rawC = _targetChroma(L, src.L, src.C, src.H, "natural"); return oklchToHex(L, _maxChromaAtLH(L, src.H, rawC), src.H); };
    solvedL = _searchL(bgLum, targetContrast, lLow, lHigh, getHex);
    if (solvedL !== null) { const rawC = _targetChroma(solvedL, src.L, src.C, src.H, "natural"); solvedC = _maxChromaAtLH(solvedL, src.H, rawC); if (solvedC < src.C - 0.01) chromaReduced = true; }

  } else {
    // natural / saturated / luminance
    const getHex = (L) => { const rawC = _targetChroma(L, src.L, src.C, src.H, solverMode); return oklchToHex(L, _maxChromaAtLH(L, src.H, rawC), src.H); };
    solvedL = _searchL(bgLum, targetContrast, lLow, lHigh, getHex);
    if (solvedL !== null) { const rawC = _targetChroma(solvedL, src.L, src.C, src.H, solverMode); solvedC = _maxChromaAtLH(solvedL, src.H, rawC); if (rawC > 0.001 && solvedC < rawC - 0.01) chromaReduced = true; }
  }

  if (solvedL === null) { // defensive — pre-check above should prevent this
    const fallback = bgIsLight ? "#000000" : "#FFFFFF";
