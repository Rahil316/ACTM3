...
describe('TEST-02 Direct Mode Solvers', () => {
  ...
  it('resolveTokensDirectly=true', () => {
    expect(config.resolveTokensDirectly).toBe(true);
  });
  ...
  it('each role has 2 variations', () => {
    for (const roleIdx of Object.keys(result.tokens['light']['Violet'])) {
      expect(Object.keys(result.tokens['light']['Violet'][roleIdx])).toHaveLength(2);
    }
  });
  ...
  it('the 5 solver-mode roles produce at least 2 distinct default values (solvers differ)', () => {
    const defaultValues = Object.values(result.tokens['light']['Violet']).map(
      (roleTokens) => (Object.values(roleTokens as Record<string, { value: string }>)[0]).value
    );
...
describe('TEST-12 Scale Algorithms', () => {
  ...
  it('different algorithms produce distinct mid-scale values for the same hue family', () => {
    ...
    const values = ['Natural', 'Uniform', 'OKLCH', 'Linear'].map(
      (name) => result.scales[name]?.[midStep]?.value
    ).filter(Boolean);
...
describe('TEST-14 Source Colors + Alpha Tints', () => {
  ...
  it('includeAlphaTints=true', () => {
    expect(config.includeAlphaTints).toBe(true);
  });
...
describe('TEST-15 Minimal Config', () => {
  ...
  it('all optional flags are off', () => {
    ...
    expect(config.includeAlphaTints).toBe(false);
    ...
    expect(config.resolveTokensDirectly).toBe(false);
