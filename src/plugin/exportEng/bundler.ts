...
export function buildExportBundle(result: EngineResult, config: ExportConfig, formats: string[], projectStore?: Record<string, unknown>): ExportFile[] {
  ...
  for (const fmt of formats) {
    ...
    if (fmt === "wand") {
      files.push({ path: `${projectSlug}.wand`, content: JSON.stringify(projectStore || {}, null, 2) });
    }
  }
