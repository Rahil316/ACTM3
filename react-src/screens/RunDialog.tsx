...
import { sendToPlugin, type SyncScope, type SyncTally, type ExistingCollection } from '../types/messages';
...
export function RunDialog() {
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      setErrorMsg('');
      setIssues([]);
      sendToPlugin({ type: 'check-collections', state: appState });
    }
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [isOpen]);

  const handleCollectionCheckResult = useCallback((msg: { existing: ExistingCollection[] }) => {
  ...
  function doRun() {
    setPhase('loading');
    sendToPlugin({ type: 'run-creator', state: appState, scope });
  }
